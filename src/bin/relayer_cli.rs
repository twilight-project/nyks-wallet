...
fn validate_btc_segwit_address(addr: &str) -> Result<(), String> {
    ...
    if !addr_str.starts_with("bc1q") {
        if addr_str.starts_with("bc1p") {
            return Err(
                "Taproot addresses (bc1p...) are not supported. Use a native SegWit address (bc1q...)."
                    .to_string(),
            );
        }
        return Err("Address must be a native SegWit address (bc1q...)".to_string());
    }
    Ok(())
}
